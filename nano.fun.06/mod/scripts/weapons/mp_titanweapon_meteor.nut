...
MeteorRadiusDamage function GetMeteorRadiusDamage( entity owner )
{
	...
	if ( owner.GetActiveWeapon() != null && owner.GetActiveWeapon().GetWeaponClassName() == "mp_titanweapon_meteor_shotgun" )
	{
		meteorRadiusDamage.pilotDamage = PLAYER_METEOR_DAMAGE_TICK_PILOT / 2
		meteorRadiusDamage.heavyArmorDamage = PLAYER_METEOR_DAMAGE_TICK / 5
	}
...
function PlayerOrNPCFire_Meteor( WeaponPrimaryAttackParams attackParams, playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
		...
		if ( weapon.GetWeaponOwner().GetActiveWeapon().GetWeaponClassName() == "mp_titanweapon_meteor_shotgun" )
		{
			float spread = 0.15
			thread ThermiteShotgunProj( attackParams, weapon, speed, spread, spread )
			thread ThermiteShotgunProj( attackParams, weapon, speed, spread, -spread )
			thread ThermiteShotgunProj( attackParams, weapon, speed, -spread, spread )
			thread ThermiteShotgunProj( attackParams, weapon, speed, -spread, -spread )
			
			thread ThermiteShotgunProj( attackParams, weapon, speed, 0.0, spread * 1.5 )
			thread ThermiteShotgunProj( attackParams, weapon, speed, 0.0, -spread * 1.5 )
			thread ThermiteShotgunProj( attackParams, weapon, speed, spread * 1.5, 0.0 )
			thread ThermiteShotgunProj( attackParams, weapon, speed, -spread * 1.5, 0.0 )
			return 1
		}
...
void function ThermiteShotgunProj( WeaponPrimaryAttackParams attackParams, entity weapon, float speed, float xspread, float yspread )
{	

	vector direction = attackParams.dir + AnglesToRight(VectorToAngles(attackParams.dir)) * xspread + AnglesToUp(VectorToAngles(attackParams.dir)) * yspread
	
	print ( direction )
		
	entity bolt = weapon.FireWeaponBolt( attackParams.pos, direction, speed, METEOR_DAMAGE_FLAGS, METEOR_DAMAGE_FLAGS, true , 0 )
	if ( bolt != null )
		EmitSoundOnEntity( bolt, "weapon_thermitelauncher_projectile_3p" )
