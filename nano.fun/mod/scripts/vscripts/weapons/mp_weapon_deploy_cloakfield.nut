...
global function MpWeaponDeployCloakfield_Init

global function OnWeaponTossReleaseAnimEvent_weapon_deploy_cloakfield
global function OnWeaponAttemptOffhandSwitch_weapon_deployable_cloakfield
global function OnWeaponTossPrep_weapon_deployable_cloakfield

/*
#if SERVER
global function DeployCloak
global function GetCloakfieldActiveCountForPlayer
#endif
*/
...
function MpWeaponDeployCloakfield_Init()
{
	PrecacheWeapon( "mp_weapon_deployable_cloak" )
}

bool function OnWeaponAttemptOffhandSwitch_weapon_deployable_cloakfield( entity weapon )
{
...
var function OnWeaponTossReleaseAnimEvent_weapon_deploy_cloakfield( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	int ammoReq = weapon.GetAmmoPerShot()
...
void function OnWeaponTossPrep_weapon_deployable_cloakfield( entity weapon, WeaponTossPrepParams prepParams )
{
...
void function OnDeployableCoverPlanted( entity projectile )
{
		...
		if ( isAmpedWall )
			Deploycloak( projectile, origin, surfaceAngles )
		else
			DeployCloakTower( projectile, origin, surfaceAngles )
	#endif
}

#if SERVER
void function DeployCloak( entity projectile, vector origin, vector angles, float duration = DEPLOYABLE_SHIELD_DURATION )
{
	Assert( IsValid( projectile ) )
...
void function DeployCloakTower( entity grenade, vector origin, vector angles )
{
	Assert( grenade )
...
int function GetCloakfieldActiveCountForPlayer( entity player )
{
	if ( !(player in file.playerToAmpedWallsActiveTable ))
		return 0
